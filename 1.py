...
def train_model(model, train_loader, test_loader, criterion, optimizer, scheduler, epochs, patience):
    ...
    final_metrics = compute_metrics(np.array(actuals), np.array(predictions))
    os.remove(best_model_path)
    return final_metrics

# Основная функция
def main():
    # Лучшие параметры из CSV
    best_params = {
        "batch_size": 64,
        "activation": "relu",
        "hidden_size": 256,
    ...
    print(f"Verifying best parameters: {best_params}")
    print(f"Expected metrics: {expected_metrics}")

    # Загрузка данных
    file_path = "LST_final_TRUE.csv"
    train_loader, test_loader = load_data(file_path, best_params["batch_size"])
    ...
    optimizer = optim.AdamW(model.parameters(), lr=best_params["learning_rate"], weight_decay=0.01)
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode="min", factor=0.1, patience=3)

    # Обучение
    mse, rmse, mae, mape, r2 = train_model(
        model, train_loader, test_loader, criterion, optimizer, scheduler,
        best_params["epochs"], patience=5
    )

    # Сравнение метрик
    achieved_metrics = {"MSE": mse, "RMSE": rmse, "MAE": mae, "MAPE": mape, "R2": r2}
    print("\nAchieved metrics:", achieved_metrics)

    # Проверка близости метрик
    for metric_name in expected_metrics:
        expected = expected_metrics[metric_name]
        achieved = achieved_metrics[metric_name]
        diff = abs(expected - achieved)
        print(f"{metric_name}: Expected={expected:.6f}, Achieved={achieved:.6f}, Difference={diff:.6f}")
...
# Achieved metrics: {'MSE': 6.356102466583252, 'RMSE': 2.5211311878962688, 'MAE': 1.889927625656128, 'MAPE': 10.490155220031738, 'R2': 0.8314369916915894}
# MSE: Expected=5.557594, Achieved=6.356102, Difference=0.798508
# RMSE: Expected=2.357455, Achieved=2.521131, Difference=0.163676
# MAE: Expected=1.751965, Achieved=1.889928, Difference=0.137963
# MAPE: Expected=9.647144, Achieved=10.490155, Difference=0.843011
# R2: Expected=0.852613, Achieved=0.831437, Difference=0.021176