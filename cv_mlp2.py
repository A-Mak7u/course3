...
X = data.drop(['T_rp5', 'Roughness', 'Aspect', 'Hillshade', 'Slope'], axis=1)
y = data['T_rp5']

# Проверка уникальности оставшихся признаков
...
# Настройка KFold (стратифицированное разбиение)
...
def create_model():
    model = Sequential()
    model.add(Dense(64, activation='tanh', input_shape=(X.shape[1],), kernel_regularizer=l2(0.001)))
...
results.to_csv('mlp_cross_validation_optimized_results.csv', index=False)