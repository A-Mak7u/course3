...
epochs_options = [100, 200, 300, 400, 500]
                ...
                for regularization in regularization_options:
                    for optimizer in optimizer_options:
                        for learning_rate in learning_rate_options:
                            mse, r2, history = run_model(epochs, batch_size, neurons, activation, regularization, optimizer, learning_rate)
                            results.append((epochs, batch_size, neurons, activation, regularization, optimizer, learning_rate, mse, r2))

# Создаем DataFrame для результатов
df_results = pd.DataFrame(results, columns=["Epochs", "Batch Size", "Neurons", "Activation", "Regularization", "Optimizer", "Learning Rate", "MSE", "R²"])
...
df_results_sorted.to_csv("model_results2.csv", index=False)
