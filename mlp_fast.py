...
# Использование mixed precision для ускорения на GPU (если поддерживается)
...
def run_model(epochs, batch_size, neurons, activation, regularization, optimizer, learning_rate):
    ...
    # Запуск с прогресс-баром tqdm
    with tqdm(total=epochs) as pbar:
        history = model.fit(X_train, y_train, epochs=epochs, batch_size=batch_size, validation_data=(X_test, y_test),
                            verbose=0)  # Без вывода в консоль
        pbar.update(epochs)  # Обновляем прогресс-бар
...
# Запуск всех комбинаций гиперпараметров
results = []
combinations = (len(epochs_options) * len(batch_size_options) * len(neurons_options) *
                len(activation_options) * len(regularization_options) * len(optimizer_options) *
                    ...
                    for regularization in regularization_options:
                        for optimizer in optimizer_options:
                            for learning_rate in learning_rate_options:
                                print(f"\nЗапуск: Эпохи={epochs}, Пакет={batch_size}, Нейроны={neurons}, "
                                      f"Активация={activation}, Регуляризация={regularization}, "
                                      f"Оптимизатор={optimizer}, LR={learning_rate}")
                                ...
                                results.append((epochs, batch_size, neurons, activation, regularization, optimizer,
                                                learning_rate, mse, r2, elapsed_time))

                                print(f"Готово! MSE={mse:.4f}, R²={r2:.4f}, Время={elapsed_time:.2f} сек\n")
